...
data class ImpactedFile(val node: GraphNode, val depth: Int, val ownDependents: Int)

data class CycleGroup(val members: List<GraphNode>)
...
object GraphInsights {
    ...
    fun impact(slice: GraphSlice, focusId: String): List<ImpactedFile> {
        val byId = slice.nodes.associateBy { it.id }
        if (focusId !in byId) return emptyList()
        val incomingTo = HashMap<String, MutableList<String>>()
        for (edge in slice.edges) {
            if (edge.from == edge.to) continue
            incomingTo.getOrPut(edge.to) { mutableListOf() } += edge.from
        }
        val depth = HashMap<String, Int>()
        depth[focusId] = 0
        val queue = ArrayDeque<String>()
        queue.addLast(focusId)
        while (queue.isNotEmpty()) {
            val current = queue.removeFirst()
            val next = depth.getValue(current) + 1
            for (predecessor in incomingTo[current].orEmpty()) {
                if (predecessor in depth) continue
                depth[predecessor] = next
        ...
        return depth.entries
            .mapNotNull { (id, d) ->
                if (id == focusId) return@mapNotNull null
                val node = byId[id] ?: return@mapNotNull null
                if (node.path == null) return@mapNotNull null
                ImpactedFile(node, d, indegree[id] ?: 0)
            }
            .sortedWith(
                compareByDescending<ImpactedFile> { it.ownDependents }
                    .thenBy { it.depth }
                    .thenBy { it.node.label.lowercase() }
                    .thenBy { it.node.id },
            )
    ...
    fun cycleGroups(slice: GraphSlice): List<CycleGroup> {
        val byId = slice.nodes.associateBy { it.id }
        return cycles(slice.edges)
            .map { ids -> CycleGroup(ids.mapNotNull { byId[it] }) }
    ...
    fun hubs(slice: GraphSlice, limit: Int = 8): List<HubFile> {
        val indegree = indegrees(slice.edges)
        return slice.nodes
            .filter { it.path != null }
            .mapNotNull { node ->
                val dependents = indegree[node.id] ?: 0
                if (dependents <= 0) null else HubFile(node, dependents)
            }
            .sortedWith(
                compareByDescending<HubFile> { it.dependents }
                    .thenBy { it.node.label.lowercase() }
                    .thenBy { it.node.id },
            )
            .take(limit)
