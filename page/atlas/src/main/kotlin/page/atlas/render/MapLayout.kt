...
private class Placed(val w: Float, val h: Float, val boxes: List<MapBox>)
...
private fun placedFile(node: GraphNode, depth: Int, activeId: String?, labelWidth: (String) -> Float): Placed {
    ...
    return Placed(w, MAP_FILE_H, listOf(box))
}
...
private fun layoutDir(
    ...
    if (dir.id !in expanded) {
        val w = labelWidth("${dir.label} ($count)") + MAP_TEXT_PAD
        val box = MapBox(
            id = dir.id,
            label = dir.label,
            folder = true,
        ...
        return Placed(w, MAP_CHIP_H, listOf(box))
    }
    ...
    return Placed(w, h, listOf(self) + shifted)
}

private fun shelfPack(items: List<Placed>): Triple<List<MapBox>, Float, Float> {
    val area = items.fold(0f) { acc, p -> acc + (p.w + MAP_GAP) * (p.h + MAP_GAP) }
    val target = max(items.maxOf { it.w }, sqrt(area) * 1.3f)
