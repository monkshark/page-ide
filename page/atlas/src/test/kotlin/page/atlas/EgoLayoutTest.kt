...
class EgoLayoutTest {
    ...
    fun `dependents sit left and imports sit right of the focus`() {
        ...
        assertTrue(model.nodes.none { it.id == "ext" }, "second-hop nodes are not drawn")
    }
    ...
    @Test
    fun `incoming edges aim at the focus and outgoing edges leave it`() {
        val model = buildEgoModel(egoSlice(), "focus")
        val incoming = model.edges.first { it.from == "dep" }
        val outgoing = model.edges.first { it.from == "focus" }

        assertEquals("focus", incoming.to)
        assertTrue(incoming.toFocus, "dependent edge points at the focus")
        assertEquals("imp", outgoing.to)
        assertTrue(!outgoing.toFocus, "import edge leaves the focus")
        assertEquals(2, model.edges.size, "only one hop is drawn on each side")
    }
    ...
    fun `imports overflow independently of dependents`() {
        ...
        assertEquals(8, column.count { it.overflow == 0 }, "import column keeps its own visible cap")
        assertEquals(12, column.first { it.overflow > 0 }.overflow)
    }
