...
Explore uses file cards with vertically centered labels, a selection ring and a Selected file label. Uses and Used by counts share the colors of the outgoing and incoming relationships; relation labels appear on selected or highlighted connections. The inspector groups source and destination, source preview and the primary open-source action. Below 880 dp, it moves beneath the graph with its own scrollbar. Fit view and zoom controls stay beside the visible-file count. Both light and dark palettes use the existing Glass theme.
